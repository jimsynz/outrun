...
fn test_session_cleanup_and_reset() {
    ...
    session.evaluate("let user = User { name: \"Alice\" }").unwrap();
    session.evaluate("struct Config(debug: Boolean) {}").unwrap();
...
fn test_session_validation_after_errors() {
    ...
    match session.validate_session_state() {
        ...
        Err(validation_error) => {
            println!("⚠️  Session validation issue after error: {}", validation_error);
...
fn test_enhanced_error_messages() {
    ...
    session.evaluate("struct TestType(field: String) {}").unwrap();
...
fn test_session_recovery_mechanisms() {
    ...
    session.evaluate("struct Original(id: Integer64) {}").unwrap();
