...
fn test_multi_step_struct_development_workflow() {
    ...
    session
        .evaluate("let alice = User { name: \"Alice\", age: 30 }")
        .unwrap();
    session
        .evaluate("let bob = User { name: \"Bob\", age: 25 }")
        .unwrap();
    ...
    session
        .evaluate("struct Team(name: String, leader: User) {}")
        .unwrap();

    // Step 5: Create composite structures
    session
        .evaluate("let team = Team { name: \"Development\", leader: alice }")
        .unwrap();
    ...
    assert!(
        session
            .evaluate("User { name: \"Charlie\", age: 35 }")
            .is_ok()
    );
...
fn test_repl_error_recovery() {
    ...
    assert!(
        session
            .evaluate("struct BadStruct { invalid syntax }")
            .is_err()
    );
...
fn test_complex_nested_struct_definitions() {
    ...
    session
        .evaluate("struct Address(street: String, city: String) {}")
        .unwrap();
    session
        .evaluate("struct Contact(email: String, phone: String) {}")
        .unwrap();

    // Step 2: Define composite struct
    session
        .evaluate("struct Person(name: String, address: Address, contact: Contact) {}")
        .unwrap();

    // Step 3: Create nested instances
    session
        .evaluate("let addr = Address { street: \"123 Main St\", city: \"NYC\" }")
        .unwrap();
    session
        .evaluate("let contact = Contact { email: \"alice@example.com\", phone: \"555-1234\" }")
        .unwrap();
    session
        .evaluate("let person = Person { name: \"Alice\", address: addr, contact: contact }")
        .unwrap();
    ...
    assert!(
        session
            .evaluate("Address { street: \"456 Oak Ave\", city: \"LA\" }")
            .is_ok()
    );
    assert!(
        session
            .evaluate("Contact { email: \"bob@example.com\", phone: \"555-5678\" }")
            .is_ok()
    );
...
fn test_session_state_management() {
    ...
    session
        .evaluate("struct Config(debug: Boolean, threads: Integer64) {}")
        .unwrap();
    session
        .evaluate("let config = Config { debug: true, threads: 4 }")
        .unwrap();
    session
        .evaluate("struct Application(name: String, config: Config) {}")
        .unwrap();
    session
        .evaluate("let app = Application { name: \"MyApp\", config: config }")
        .unwrap();
    ...
    assert!(
        session
            .evaluate("Config { debug: false, threads: 8 }")
            .is_err()
    );
    assert!(
        session
            .evaluate("Application { name: \"Test\", config: Config { debug: true, threads: 2 } }")
            .is_err()
    );
...
fn test_integer_arithmetic_with_struct_persistence() {
    ...
    session
        .evaluate("struct Calculator(value: Integer64) {}")
        .unwrap();

    // Step 2: Create calculator instances with arithmetic
    session
        .evaluate("let calc1 = Calculator { value: 10 + 5 }")
        .unwrap();
    session
        .evaluate("let calc2 = Calculator { value: 20 * 2 }")
        .unwrap();
    session
        .evaluate("let calc3 = Calculator { value: 100 - 25 }")
        .unwrap();
...
fn test_multiple_session_workflows() {
    ...
    session1
        .evaluate("struct TypeA(field1: String) {}")
        .unwrap();
    session2
        .evaluate("struct TypeB(field2: Integer64) {}")
        .unwrap();

    // Create instances in each session
    session1
        .evaluate("let instance_a = TypeA { field1: \"session1\" }")
        .unwrap();
    session2
        .evaluate("let instance_b = TypeB { field2: 42 }")
        .unwrap();
...
fn test_session_performance_under_load() {
    ...
    session
        .evaluate("struct Node(id: Integer64, value: String) {}")
        .unwrap();
    session
        .evaluate("struct Edge(from: Integer64, to: Integer64, weight: Integer64) {}")
        .unwrap();
    ...
    assert!(
        session
            .evaluate("Node { id: 999, value: \"new_node\" }")
            .is_ok()
    );
    assert!(
        session
            .evaluate("Edge { from: 999, to: 1000, weight: 50 }")
            .is_ok()
    );
