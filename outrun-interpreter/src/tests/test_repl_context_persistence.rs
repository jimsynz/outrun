...
fn test_basic_struct_definition_persistence() {
    ...
    match result {
        Ok(value) => {
            println!("✅ Struct usage and assignment succeeded: {}", value.display());
...
fn test_struct_definition_registry_tracking() {
    ...
    session.evaluate("struct Product(id: Integer64, name: String) {}").unwrap();
    ...
    match (user_result, product_result) {
        ...
        (user_res, product_res) => {
            if user_res.is_err() { println!("❌ User struct failed: {:?}", user_res); }
            if product_res.is_err() { println!("❌ Product struct failed: {:?}", product_res); }
        }
    }
}