...
fn test_automatic_equality_runtime_behavior() {
    ...
    harness.evaluate("struct Point(x: Integer64, y: Integer64) {}").unwrap();
    ...
    harness.assert_evaluates_to_value("p1", harness.get_variable("p1").unwrap().clone()).unwrap();
    harness.assert_evaluates_to_value("p2", harness.get_variable("p2").unwrap().clone()).unwrap();
    harness.assert_evaluates_to_value("p3", harness.get_variable("p3").unwrap().clone()).unwrap();
...
fn test_equality_override_runtime_behavior() {
    ...
    harness.evaluate("struct Person(name: String, age: Integer64) {}").unwrap();
    
    // Test that we can create Person instances
    harness.evaluate("let alice = Person { name: \"Alice\", age: 30 }").unwrap();
    harness.evaluate("let bob = Person { name: \"Bob\", age: 25 }").unwrap();
    harness.evaluate("let alice2 = Person { name: \"Alice\", age: 30 }").unwrap();
    ...
    harness.assert_evaluates_to_value("alice", harness.get_variable("alice").unwrap().clone()).unwrap();
    harness.assert_evaluates_to_value("bob", harness.get_variable("bob").unwrap().clone()).unwrap();
    harness.assert_evaluates_to_value("alice2", harness.get_variable("alice2").unwrap().clone()).unwrap();
    
    // Test that the struct definition persisted and can still be used
    harness.evaluate("Person { name: \"Charlie\", age: 35 }").unwrap();
