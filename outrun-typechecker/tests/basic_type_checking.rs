...
use outrun_typechecker::{typecheck_program, TypeChecker};
...
fn test_simple_literal_program() {
    ...
    match result {
        ...
        Err(errors) => {
            ...
            println!("Expected error for unimplemented features: {:?}", errors);
        }
    }
}
...
fn test_type_checking_pipeline() {
    ...
    match result {
        ...
        Err(errors) => println!("Type checking failed (expected): {:?}", errors),
    }
}
