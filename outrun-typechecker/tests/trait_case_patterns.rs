...
use outrun_typechecker::checker::{ExpressionChecker, TypeContext};
use outrun_typechecker::error::TypeError;
use outrun_typechecker::types::traits::{TraitDefinition, TraitFunction, TraitImplementation};
use outrun_typechecker::types::{ConcreteType, StructField};
use std::collections::HashMap;

fn setup_test_context_with_structs() -> (
    TypeContext,
    outrun_typechecker::types::TraitId,
    outrun_typechecker::types::TypeId,
    outrun_typechecker::types::TypeId,
) {
...
fn test_valid_struct_pattern_destructuring() {
    ...
    let value_var = outrun_typechecker::checker::Variable {
        name: "value".to_string(),
        type_id: value_type,
        is_mutable: false,
        span: Span::new(0, 5),
...
fn test_pattern_with_wrong_field() {
    ...
    match result.unwrap_err() {
        ...
        other => panic!("Expected TypeMismatch error about field, got: {:?}", other),
    }
}
