...
use outrun_typechecker::{
    error::{
        CompilerError, DispatchError, ErrorContext, InferenceError, TypecheckError,
        UnificationError,
    },
    types::Type,
};
...
def login(usename: String, password: String): Result<User, LoginError> {
    ...
    let report = Report::new(CompilerError::Typecheck(TypecheckError::InferenceError(
        error,
    )))
...
"#;
    ...
    let report = Report::new(CompilerError::Typecheck(TypecheckError::UnificationError(
        error,
    )))
    .with_source_code(named_source);
...
fn demo_missing_protocol_implementation() {
    ...
    let error = DispatchError::no_implementation_with_suggestions(
        ...
        Some(Span::new(15, 16)),
        vec![
            "BinaryAddition for Integer64".to_string(),
            "BinaryAddition for Float64".to_string(),
        ],
    );
...
"#;
    ...
    let report = Report::new(CompilerError::Typecheck(TypecheckError::DispatchError(
        error,
    )))
    .with_source_code(named_source);
...
"#;
    ...
    let report1 = Report::new(CompilerError::Typecheck(TypecheckError::InferenceError(
        var_error,
    )))
    ...
    println!("\nType error:");
    let report2 = Report::new(CompilerError::Typecheck(TypecheckError::InferenceError(
        type_error,
    )))
    .with_source_code(named_source);
    println!("{report2:?}");
