...
mod tests {
    ...
    #[test]
    fn test_introspection_registry_creation() {
        let registry = IntrospectionRegistry::new();
        assert_eq!(registry.struct_types.len(), 0);
        assert_eq!(registry.trait_types.len(), 0);
    }
    ...
    fn test_struct_type_registration() {
        ...
        let struct_info = StructTypeInfo {
            ...
            fields: vec![StructFieldInfo {
                name: "name".to_string(),
                field_type: string_type_id,
                span: outrun_parser::Span::new(0, 4),
            }],
            implemented_traits: vec![],
            span: outrun_parser::Span::new(0, 20),
        };

        registry.register_struct_type(user_type_id, struct_info);

        assert!(registry.is_struct_type(user_type_id));
        assert!(!registry.is_trait_type(user_type_id));
        assert_eq!(registry.get_type_kind(user_type_id), Some(TypeKind::Struct));
        assert_eq!(registry.get_type_name(user_type_id), Some("User"));
        assert_eq!(registry.get_type_module(user_type_id), Some("App"));
    }
    ...
    fn test_trait_type_registration() {
        ...
        let display_type_id = interner.intern_type("Display");

        let trait_info = TraitTypeInfo {
            name: "Display".to_string(),
            module: "Core".to_string(),
            functions: vec![],
            constraints: vec![],
            span: outrun_parser::Span::new(0, 15),
        };

        registry.register_trait_type(display_type_id, trait_info);

        assert!(registry.is_trait_type(display_type_id));
        assert!(!registry.is_struct_type(display_type_id));
        assert_eq!(
            registry.get_type_kind(display_type_id),
            Some(TypeKind::Trait)
        );
        assert_eq!(registry.get_type_name(display_type_id), Some("Display"));
        assert_eq!(registry.get_type_module(display_type_id), Some("Core"));
    }
    ...
    fn test_trait_implementation_checking() {
        ...
        let user_type_id = interner.intern_type("User");
        let display_type_id = interner.intern_type("Display");
        let debug_type_id = interner.intern_type("Debug");

        let struct_info = StructTypeInfo {
            name: "User".to_string(),
            module: "App".to_string(),
            fields: vec![],
            implemented_traits: vec![display_type_id], // User implements Display
            span: outrun_parser::Span::new(0, 10),
        };

        registry.register_struct_type(user_type_id, struct_info);

        assert!(registry.struct_implements_trait(user_type_id, display_type_id));
        assert!(!registry.struct_implements_trait(user_type_id, debug_type_id));
    }
}
