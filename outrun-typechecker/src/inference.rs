...
impl TypeInferenceEngine {
    ...
    fn infer_local_function_call(
        ...
        match dispatcher.resolve_local_call(function_name, Some(name.span)) {
            Ok(dispatch_result) => self.handle_dispatch_result(dispatch_result, arguments, context),
            Err(dispatch_error) => {
        ...
        match dispatcher.resolve_qualified_call(&qualified_name, None, Some(name.span)) {
            ...
            Err(dispatch_error) => {
                ...
                if module.name == "Outrun.Intrinsic" {
                    match self
                        .try_lazy_intrinsic_registration(&name.name, arguments, name.span, context)
                    {
                    ...
                    if self.is_protocol_type(&type_name) {
                        ...
                        if type_name == "Boolean" {
                            ...
                            eprintln!(
                                ...
                                self.protocol_registry.has_protocol(&boolean_protocol_id)
                            );
                            eprintln!(
                                "   - This will create: Concrete {{ id: TypeId(\"{}\") }}",
                                type_name
                            );
                        }
                        Ok(Type::Concrete {
