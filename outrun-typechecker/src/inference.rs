...
impl TypeInferenceEngine {
    ...
    pub fn function_registry_mut(&mut self) -> &mut FunctionRegistry {
        ...
        Rc::get_mut(&mut self.function_registry).expect("Should be uniquely owned after replacement")
    }
    ...
    pub fn create_generic_context_from_names(&mut self, param_names: &[String]) -> HashMap<String, Type> {
    ...
    #[allow(clippy::result_large_err)]
    pub fn typecheck_program_items_only(&mut self, program: &mut Program) -> Result<(), TypecheckError> {
    ...
    #[allow(clippy::result_large_err)]
    /// Phase 2: Register all protocols and structs (definitions only)
    pub fn register_protocols_and_structs(&mut self, program: &Program) -> Result<(), TypecheckError> {
        ...
        match &item.kind {
            ...
            ItemKind::ProtocolDefinition(protocol_def) => {
                ...
                for protocol_function in &protocol_def.functions {
                    self.register_protocol_function_signature(&protocol_name, protocol_function)?;
                }
            }
            // Skip expression items and other items
            _ => {}
    ...
    /// Phase 2.5: Register automatic implementations (Any, Inspect) for all struct types
    #[allow(clippy::result_large_err)]
    pub fn register_automatic_implementations(&mut self, program: &Program) -> Result<(), TypecheckError> {
    ...
    /// Phase 6: Type check all function bodies with complete context
    #[allow(clippy::result_large_err)]
    pub fn typecheck_function_bodies(&mut self, program: &mut Program) -> Result<(), TypecheckError> {
    ...
    fn collect_struct_definition(&mut self, struct_def: &StructDefinition) -> Result<(), TypecheckError> {
        ...
        self.struct_registry.insert(struct_name.clone(), struct_def.clone());
    ...
    fn collect_protocol_definition(&mut self, protocol_def: &ProtocolDefinition) -> Result<(), TypecheckError> {
        ...
        self.protocol_registry_mut().add_local_module(protocol_module);
    ...
    #[allow(clippy::result_large_err)]
    fn register_protocol_function_signature(&mut self, protocol_name: &str, protocol_function: &ProtocolFunction) -> Result<(), TypecheckError> {
        use outrun_parser::ProtocolFunction;
        
        match protocol_function {
            ProtocolFunction::Signature(signature) => {
                self.register_protocol_signature(protocol_name, signature)?;
            }
            ProtocolFunction::Definition(definition) => {
                // For protocol function definitions, register the signature part
                self.register_protocol_function_definition(protocol_name, definition)?;
            }
            ProtocolFunction::StaticDefinition(static_def) => {
                // For static definitions, register the signature part
                self.register_protocol_static_definition(protocol_name, static_def)?;
            }
        }
    ...
    #[allow(clippy::result_large_err)]
    fn register_protocol_signature(&mut self, protocol_name: &str, signature: &FunctionSignature) -> Result<(), TypecheckError> {
        // Extract function name
    ...
    #[allow(clippy::result_large_err)]
    fn register_protocol_function_definition(&mut self, protocol_name: &str, definition: &FunctionDefinition) -> Result<(), TypecheckError> {
        // Extract function name
        ...
        // Extract parameters
        let parameters: Result<Vec<(String, Type)>, TypecheckError> = definition.parameters
            .iter()
            .map(|param| {
                let param_name = param.name.name.clone();
                let param_type = self.convert_type_annotation(&param.type_annotation)?;
    ...
    #[allow(clippy::result_large_err)]
    fn register_protocol_static_definition(&mut self, protocol_name: &str, static_def: &StaticFunctionDefinition) -> Result<(), TypecheckError> {
        // Extract function name
    ...
    fn collect_impl_block_functions(&mut self, impl_block: &ImplBlock) -> Result<(), TypecheckError> {
        ...
        // Extract generic parameters from the impl block declaration (impl<T, U>)
        let impl_generic_params = if let Some(generic_params) = &impl_block.generic_params {
            generic_params.params.iter().map(|param| param.name.name.clone()).collect()
        } else {
            Vec::new()
        };
        ...
        let implementing_args: Vec<Type> = impl_generic_params.iter()
    ...
    fn extract_protocol_generic_parameters(&self, protocol_def: &ProtocolDefinition) -> Vec<String> {
    ...
    #[allow(clippy::result_large_err)]
    fn register_automatic_implementations_for_struct(&mut self, struct_def: &StructDefinition) -> Result<(), TypecheckError> {
        // Extract struct name (handle nested names like Module.SubModule.StructName)
        let struct_name = struct_def.name
            .iter()
            .map(|segment| segment.name.as_str())
    ...
    fn register_impl_block(&mut self, impl_block: &ImplBlock) -> Result<(), TypecheckError> {
        ...
        let protocol_name = impl_block.protocol_spec.path
            .iter()
            .map(|segment| segment.name.as_str())
            .collect::<Vec<_>>()
            .join(".");
        ...
        let implementing_type_name = impl_block.type_spec.path
            .iter()
            .map(|segment| segment.name.as_str())
            .collect::<Vec<_>>()
            .join(".");
        ...
        self.protocol_registry_mut().add_local_module(module_id.clone());
    ...
    fn types_are_compatible(&self, found_type: &Type, expected_type: &Type) -> bool {
        ...
        match (found_type, expected_type) {
            ...
            (Type::Protocol { id: protocol, .. }, Type::Concrete { id: concrete_type, .. }) => {
            ...
            (Type::Protocol { id: id1, args: args1, .. }, Type::Protocol { id: id2, args: args2, .. }) => {
                ...
                if id1 == id2 && args1.len() == args2.len() {
                    ...
                    args1.iter().zip(args2.iter()).all(|(arg1, arg2)| {
                        self.types_are_compatible(arg1, arg2)
                    })
                } else {
                    false
                }
    ...
    pub fn infer_expression(
        ...
        match &expression.kind {
            ...
            ExpressionKind::FieldAccess(field_access) => {
                ...
                self.infer_field_access_type(&object_result.inferred_type, &field_access.field.name, context)
            }
        ...
        match dispatcher.resolve_qualified_call(&qualified_name, None, Some(name.span)) {
            ...
            Err(dispatch_error) => {
                ...
                if module.name == "Outrun.Intrinsic" {
                    match self.try_lazy_intrinsic_registration(&name.name, arguments, name.span, context) {
    ...
    fn try_lazy_intrinsic_registration(
        ...
        let (return_type, parameter_types) = match intrinsic_name {
            ...
            "map_get" if arguments.len() == 2 => {
                ...
                let option_any_type = Type::protocol_with_args_and_span("Option", vec![any_type.clone()], call_span);
                (
                    option_any_type,
                    vec![
            ...
            "string_length" if arguments.len() == 1 => {
                ...
                (
                    integer_type,
                    vec![("value".to_string(), string_type)]
                )
            }
            ...
            "string_char_at" if arguments.len() == 2 => {
                let string_type = Type::concrete_with_span("Outrun.Core.String", call_span);
                let integer_type = Type::protocol_with_span("Integer", call_span);
                let option_string_type = Type::protocol_with_args_and_span("Option", vec![string_type.clone()], call_span);
                (
                    option_string_type,
                    vec![
            ...
            "string_index_of" if arguments.len() == 2 => {
                let string_type = Type::concrete_with_span("Outrun.Core.String", call_span);
                let integer_type = Type::protocol_with_span("Integer", call_span);
                let option_integer_type = Type::protocol_with_args_and_span("Option", vec![integer_type], call_span);
                (
                    option_integer_type,
                    vec![
                        ("value".to_string(), string_type.clone()),
            ...
            "string_to_upper" | "string_to_lower" | "string_trim" | "string_trim_start" | "string_trim_end" 
                if arguments.len() == 1 => {
                let string_type = Type::concrete_with_span("Outrun.Core.String", call_span);
                (
                    string_type.clone(),
            ...
            "binary_byte_at" if arguments.len() == 2 => {
                let any_type = Type::protocol_with_span("Any", call_span);
                let integer_type = Type::protocol_with_span("Integer", call_span);
                let option_integer_type = Type::protocol_with_args_and_span("Option", vec![integer_type.clone()], call_span);
                (
                    option_integer_type,
                    vec![
            ...
            "binary_from_hex" if arguments.len() == 1 => {
                let string_type = Type::concrete_with_span("Outrun.Core.String", call_span);
                let any_type = Type::protocol_with_span("Any", call_span);
                let option_any_type = Type::protocol_with_args_and_span("Option", vec![any_type], call_span);
                (
                    option_any_type,
                    vec![("value".to_string(), string_type)]
                )
            ...
            "panic" if arguments.len() == 1 => {
                ...
                (
                    panic_type,
                    vec![("message".to_string(), string_type)]
                )
            }
            ...
            _ => {
                return Err(TypecheckError::InferenceError(
                    crate::error::InferenceError::AmbiguousType {
                        span: None,
                        suggestions: vec![format!("Unknown intrinsic function: {}", intrinsic_name)],
                    }
                ));
            }
        };
    ...
    fn infer_qualified_identifier(
        ...
        if let Some(function_info) = self.function_registry.get_function(module_name, function_name) {
    ...
    pub(crate) fn convert_type_annotation(&mut self, type_annotation: &TypeAnnotation) -> Result<Type, TypecheckError> {
            ...
            TypeAnnotation::Simple { path, generic_args, .. } => {
                ...
                // Process generic arguments if present
                    ...
                    if self.is_protocol_type(&type_name) {
                        ...
                        // DEBUG: Catch when Boolean is incorrectly treated as concrete during core library loading
                        if type_name == "Boolean" {
                            eprintln!("🐛 DEBUG: Boolean being treated as CONCRETE type during convert_type_annotation!");
                            eprintln!("   - is_protocol_type({}) returned false", type_name);
                            let boolean_protocol_id = crate::types::ProtocolId::new("Boolean");
                            eprintln!("   - Boolean protocol registered: {}", self.protocol_registry.has_protocol(&boolean_protocol_id));
                            eprintln!("   - This will create: Concrete {{ id: TypeId(\"{}\") }}", type_name);
                        }
                        Ok(Type::Concrete {
    ...
    fn type_ids_are_equivalent(&self, id1: &crate::types::TypeId, id2: &crate::types::TypeId) -> bool {
        id1 == id2
    }
        ...
        if let Some(struct_def) = self.struct_registry.get(struct_name).cloned() {
            ...
            for field in &struct_def.fields {
                if field.name.name == field_name {
                    ...
                    let resolved_field_type = if !struct_args.is_empty() && !struct_def.generic_params.as_ref().map_or(true, |gp| gp.params.is_empty()) {
                        self.substitute_generic_args_in_field_type(field_type, &struct_def, struct_args)?
                    } else {
                        field_type
                    };
            ...
            let available_fields: Vec<String> = struct_def.fields.iter()
    ...
    fn substitute_generic_args_in_field_type(
        ...
        let generic_param_names: Vec<String> = if let Some(generic_params) = &struct_def.generic_params {
        ...
        match ty {
            Type::Concrete { id, args, span } => {
                ...
                if let Some(substituted_type) = substitution.get(id.name()) {
                    ...
                    let substituted_args: Result<Vec<Type>, TypecheckError> = args.into_iter()
