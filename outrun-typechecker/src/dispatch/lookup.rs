...
impl<'a> DispatchTableBuilder<'a> {
    ...
    fn build_trait_dispatch(&mut self) -> TypeResult<()> {
        ...
        {
            eprintln!(
                "Building trait dispatch with {} implementations",
                implementations.len()
            );
        }
        ...
        for impl_block in implementations {
            ...
            // Log dispatch registration for debugging (if needed)
            #[cfg(debug_assertions)]
            {
                eprintln!(
                    "Registered dispatch: trait {:?} for type {:?} -> module {:?}",
                    impl_block.trait_id, impl_block.type_id, module_id
                );
            }
        }
            ...
            for trait_func in &trait_def.functions {
                if trait_func.is_static {
                    ...
                    if let Some(function_name) = self.interner.atom_name(trait_func.name) {
                        ...
                        // Log static function registration for debugging
                        #[cfg(debug_assertions)]
                        {
                            eprintln!(
                                "Registered static function: {} -> {} (id: {:?})",
                                trait_def.name, function_name, function_id
                            );
                        }
                    }
                }
...
mod tests {
    ...
    fn test_dispatch_builder_creation() {
        ...
        let builder = DispatchTableBuilder::new(&registry, &mut interner);
