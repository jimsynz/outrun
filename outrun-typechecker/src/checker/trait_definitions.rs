...
mod tests {
    ...
    fn test_simple_trait_definition() {
        ...
        assert!(
            result.is_ok(),
            "Simple trait definition should type check: {:?}",
            result
        );
    ...
    fn test_trait_with_constraints() {
        ...
        assert!(
            result.is_ok(),
            "Trait with constraints should type check: {:?}",
            result
        );
    ...
    fn test_trait_with_guard_function() {
        ...
        assert!(
            result.is_ok(),
            "Trait with guard function should type check: {:?}",
            result
        );
    ...
    fn test_invalid_guard_function_return_type() {
        ...
        assert!(
            result.is_err(),
            "Invalid guard function should fail type checking"
        );
        ...
        match &errors[0] {
            TypeError::InvalidGuardFunction {
                function_name,
                actual_return_type,
                ..
            } => {
    ...
    fn test_undefined_type_parameter_in_constraint() {
        ...
        let program =
            parse_program(source).expect("Should parse trait with undefined type parameter");
        let result = checker.check_program(&program);

        assert!(
            result.is_err(),
            "Undefined type parameter should fail type checking"
        );
        ...
        match &errors[0] {
            ...
            _ => panic!(
                "Expected UndefinedTypeParameter error, got: {:?}",
                errors[0]
            ),
        }
    }
    ...
    fn test_trait_with_multiple_functions() {
        ...
        assert!(
            result.is_ok(),
            "Trait with multiple functions should type check: {:?}",
            result
        );
        ...
        assert!(trait_def
            .find_function(is_empty_atom)
            .unwrap()
            .is_guard_function());
        assert!(trait_def
            .find_function(contains_atom)
            .unwrap()
            .is_guard_function());
    }
    ...
    fn test_trait_with_complex_constraints() {
        ...
        assert!(
            result.is_ok(),
            "Trait with complex constraints should type check: {:?}",
            result
        );
