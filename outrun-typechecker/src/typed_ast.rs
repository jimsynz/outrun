...
impl TypedExpression {
    ...
    pub fn get_clause_ids(&self) -> Option<&[ClauseId]> {
        match &self.typed_kind {
            TypedExpressionKind::FunctionCall { possible_clauses, .. } => Some(possible_clauses),
            _ => None,
        }
    }
    ...
    pub fn is_ambiguous_call(&self) -> bool {
        match &self.typed_kind {
            TypedExpressionKind::FunctionCall { possible_clauses, .. } => possible_clauses.len() > 1,
            _ => false,
        }
    }
}
...
impl TypedArgument {
    ...
    pub fn new(parser_argument: outrun_parser::Argument, typed_expression: TypedExpression) -> Self {
        Self {
            parser_argument,
...
mod tests {
    ...
    fn test_universal_call_resolution_multi() {
        let clause_id1 = ClauseId::new();
        let clause_id2 = ClauseId::new();
        let return_type = crate::types::Type::concrete("Boolean");
        
        let resolution = UniversalCallResolution::multi(
            vec![clause_id1, clause_id2], 
            return_type.clone()
        );
    ...
    fn test_typed_expression_ambiguous_call() {
        ...
        let clause_id1 = ClauseId::new(); // Some<T> implementation
        let clause_id2 = ClauseId::new(); // None<T> implementation
        let return_type = crate::types::Type::concrete("Boolean");
        let resolution = UniversalCallResolution::multi(
            vec![clause_id1, clause_id2], 
            return_type.clone()
        ...
        assert!(typed_expr.is_universal_function_call());
        assert!(typed_expr.is_ambiguous_call());
        assert_eq!(typed_expr.get_clause_ids(), Some([clause_id1, clause_id2].as_slice()));
        assert_eq!(typed_expr.inferred_type, return_type);
    }
}