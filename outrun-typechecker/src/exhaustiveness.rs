...
mod tests {
    ...
    fn test_boolean_exhaustiveness_complete() {
        ...
        let result = analyzer
            .analyze_boolean_exhaustiveness(&covered_patterns, Span::new(0, 10))
            .unwrap();
    ...
    #[test]
    fn test_boolean_exhaustiveness_missing_false() {
        let context = setup_test_context();
        let analyzer = ExhaustivenessAnalyzer::new(&context);
        ...
        match result {
            ...
            _ => panic!("Expected missing patterns, got: {:?}", result),
        }
    }

    #[test]
    fn test_boolean_exhaustiveness_missing_both() {
        let context = setup_test_context();
        let analyzer = ExhaustivenessAnalyzer::new(&context);
        ...
        match result {
            ExhaustivenessResult::Missing(missing) => {
                ...
                // Should have both true and false missing
                let descriptions: Vec<&String> = missing.iter().map(|m| &m.description).collect();
                assert!(descriptions.contains(&&"Boolean value 'true'".to_string()));
                assert!(descriptions.contains(&&"Boolean value 'false'".to_string()));
            }
            _ => panic!("Expected missing patterns, got: {:?}", result),
        }
    }
    ...
    #[test]
    fn test_trait_case_exhaustiveness() {
        let mut context = setup_test_context();

        // Set up a simple trait with implementing types
        let my_trait_id = context.interner.intern_trait("MyTrait");
        let type_a_id = context.interner.intern_type("TypeA");
        let type_b_id = context.interner.intern_type("TypeB");
        ...
        let trait_def =
            TraitDefinition::new(my_trait_id, "MyTrait".to_string(), vec![], Span::new(0, 10));
        context.trait_registry.register_trait(trait_def);

        let impl_a =
            TraitImplementation::new(my_trait_id, type_a_id, HashMap::new(), Span::new(0, 10));
        let impl_b =
            TraitImplementation::new(my_trait_id, type_b_id, HashMap::new(), Span::new(0, 10));

        context.trait_registry.register_implementation(impl_a);
        context.trait_registry.register_implementation(impl_b);
        ...
        let covered_patterns = vec![
            CoveredPattern::TraitType(type_a_id),
            CoveredPattern::TraitType(type_b_id),
        ];
        let case_type = CaseType::Trait(my_trait_id);

        let result = analyzer
            .analyze_case_exhaustiveness(case_type, &covered_patterns, Span::new(0, 10))
            .unwrap();
