...
fn test_phase4_dependency_template_import() {
    ...
    match result {
        Ok(imported_count) => {
            println!("✓ Successfully imported {} dependency templates", imported_count);
            ...
            assert!(stored_templates.contains_key(&sig1), "Should contain first template");
            
            let imported_template1 = &stored_templates[&sig1];
            assert_eq!(imported_template1.generic_parameters.len(), 1, "Template 1 should have 1 generic parameter");
            assert_eq!(imported_template1.parameter_types.len(), 1, "Template 1 should have 1 parameter");
            assert_eq!(imported_template1.source_package, "dependency_pkg", "Template 1 should have correct package name");
            ...
            assert!(stored_templates.contains_key(&sig2), "Should contain second template");
            
            let imported_template2 = &stored_templates[&sig2];
            assert_eq!(imported_template2.generic_parameters.len(), 0, "Template 2 should have no generic parameters");
            assert_eq!(imported_template2.parameter_types.len(), 2, "Template 2 should have 2 parameters");
            assert_eq!(imported_template2.source_package, "dependency_pkg", "Template 2 should have correct package name");
...
fn test_phase4_clause_generation_from_template() {
    ...
    let template = crate::constraints::PublicFunctionTemplate {
        function_signature: crate::universal_dispatch::FunctionSignature::simple("generic_function".to_string()),
        generic_parameters: vec![
            crate::constraints::GenericParameter {
                name: "T".to_string(),
    ...
    match result {
        Ok(clause_info) => {
            ...
            println!("  Function: {}", clause_info.function_signature.function_name);
            println!("  Guards: {}", clause_info.guards.len());
            
            // Verify clause structure
            assert_eq!(clause_info.function_signature.function_name, "generic_function");
            // Template-generated clauses should have guards for the type template information
            assert!(clause_info.guards.len() > 0, "Template-generated clauses should have guards for type checking");
...
fn test_phase4_type_substitution_helpers() {
    ...
    let template = crate::constraints::PublicFunctionTemplate {
        function_signature: crate::universal_dispatch::FunctionSignature::simple("test_substitution".to_string()),
        generic_parameters: vec![
            crate::constraints::GenericParameter {
                name: "T".to_string(),
                direct_constraints: vec![],
    ...
    match result {
        Ok(clause_info) => {
            ...
            println!("  Generated clause for function: {}", clause_info.function_signature.function_name);
...
fn test_phase4_comprehensive_integration() {
    ...
    let imported_template = &dependency_templates[&crate::universal_dispatch::FunctionSignature::qualified(
        "ExternalLib".to_string(),
        "transform".to_string()
    )];
    
    let clause_result = solver.generate_clause_from_template(imported_template, &type_substitutions);
    ...
    assert_eq!(generated_clause.function_signature.module_path, vec!["ExternalLib".to_string()]);
    assert_eq!(generated_clause.function_signature.function_name, "transform");
...
fn test_phase4_visibility_restriction() {
    ...
    dependency_templates.insert(private_template.function_signature.clone(), private_template);
    ...
    match result {
        Ok(imported_count) => {
            println!("✓ Import completed with {} templates imported", imported_count);
            
            // Should only import 1 template (the public one)
            assert_eq!(imported_count, 1, "Should only import 1 public template, not private ones");
            
            // Verify that only the public template was stored
            let stored_templates = solver.get_public_function_templates();
            assert_eq!(stored_templates.len(), 1, "Should have exactly 1 stored template");
            ...
            assert!(stored_templates.contains_key(&public_sig), "Should contain public function");
            ...
            assert!(!stored_templates.contains_key(&private_sig), "Should NOT contain private function");
