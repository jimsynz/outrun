...
mod generic_parameter_resolution_tests {
    ...
    use crate::types::{Type, ProtocolId, TypeVarId, Level, ModuleId};
    ...
    fn test_impl_block_generic_parameter_context() {
        ...
        assert!(result.is_ok(), "Implementation registration should succeed with generic parameters");
    }
    ...
    fn test_generic_parameter_type_variable_creation() {
        ...
        for param_name in &param_names {
            let param_type = context.get(param_name).expect("Parameter should exist in context");
            match param_type {
                Type::Variable { var_id: _, level: Level(0), name: Some(name), span: None } => {
                    assert_eq!(name, param_name, "Variable name should match parameter name");
                }
                _ => panic!("Generic parameter should be a type variable, got: {:?}", param_type),
            }
        }
    }
    ...
    fn test_convert_type_annotation_with_generics() {
        ...
        engine.add_generic_parameter("T".to_string(), Type::Variable {
            var_id: TypeVarId(42),
            level: Level(0),
        ...
        if let Some(item) = program.items.first() {
            if let outrun_parser::ItemKind::FunctionDefinition(func_def) = &item.kind {
                ...
                let param_type_result = engine.convert_type_annotation(&func_def.parameters[0].type_annotation);
                assert!(param_type_result.is_ok(), "Should convert T to type variable");
                
                let param_type = param_type_result.unwrap();
                match param_type {
                    Type::Variable { var_id: TypeVarId(42), .. } => {
                        // Success - T was resolved to the type variable we set up
                    }
                    _ => panic!("T should resolve to the type variable in context, got: {:?}", param_type),
                }
                
                // Convert the return type annotation  
                let return_type_result = engine.convert_type_annotation(&func_def.return_type);
                assert!(return_type_result.is_ok(), "Should convert return T to type variable");
                ...
                match return_type {
                    ...
                    _ => panic!("Return T should resolve to the same type variable, got: {:?}", return_type),
                }
            }
        }
    }
    ...
    fn test_generic_type_with_arguments() {
        ...
        if let Some(item) = program.items.first() {
            if let outrun_parser::ItemKind::FunctionDefinition(func_def) = &item.kind {
                ...
                let param_type_result = engine.convert_type_annotation(&func_def.parameters[0].type_annotation);
                assert!(param_type_result.is_ok(), "Should convert Option<T> successfully");
                
                let param_type = param_type_result.unwrap();
                match param_type {
                    Type::Protocol { id: ProtocolId(ref protocol_name), ref args, .. } => {
                        ...
                        match &args[0] {
                            Type::Variable { var_id: TypeVarId(99), .. } => {
                                // Success - T was resolved correctly within Option<T>
                            }
                            _ => panic!("Option<T> argument should be the T type variable, got: {:?}", args[0]),
                        }
                    }
