...
fn test_option_integer_constraint_compatibility() {
    ...
    let option_integer_constraint = Type::protocol_with_args(
        "Option",
        vec![Type::protocol("Integer")]
    );
    
    // Create Option<Outrun.Core.Integer64> - a concrete instantiation
    let option_concrete_integer = Type::protocol_with_args(
        "Option", 
        vec![Type::concrete("Outrun.Core.Integer64")]
    );
    
    println!("Option<Integer> constraint: {:?}", option_integer_constraint);
    println!("Option<Outrun.Core.Integer64> concrete: {:?}", option_concrete_integer);
    ...
    let is_compatible = engine.test_types_are_compatible(
...
fn test_option_integer_constraint_negative_case() {
    ...
    let option_string = Type::protocol_with_args(
        "Option",
        vec![Type::concrete("Outrun.Core.String")]
    );
