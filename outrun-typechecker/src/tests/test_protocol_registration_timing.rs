...
"#;
    ...
    let mut protocol_program = parse_program(integer_protocol).expect("Parse protocol should succeed");
    engine2.register_protocols_and_structs(&protocol_program).expect("Should register Integer protocol");
    ...
    println!("=== This demonstrates that the issue occurs during type checking, not registration ===");
}