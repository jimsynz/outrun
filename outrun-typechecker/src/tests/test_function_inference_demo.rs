...
fn test_function_inference_success_cases() {
    ...
    let function_type_program =
        r#"def process(callback: Function<(x: Integer) -> String>): String { "result" }"#;
            ...
            if let Some(item) = program.items.first() {
                if let outrun_parser::ItemKind::Expression(expr) = &item.kind {
                    if let ExpressionKind::AnonymousFunction(anon_fn) = &expr.kind {
                        println!(
                            "✅ PARSED: Anonymous function with {} clauses",
                            anon_fn.clauses.len()
                        );
...
fn test_function_type_display_and_structure() {
    ...
    assert!(
        display_string.contains("Function"),
        "Should contain 'Function'"
    );
    assert!(
        display_string.contains("x: Integer"),
        "Should show parameter x"
    );
    assert!(
        display_string.contains("y: String"),
        "Should show parameter y"
    );
    assert!(
        display_string.contains("-> String"),
    ...
    assert!(
        no_param_display.contains("Function<() -> Integer"),
        "Should show no-param syntax"
    );
}
...
fn test_function_inference_error_cases() {
    ...
    let error_test_cases = vec![
        ...
        (
            "Invalid syntax",
            r#"def test(x: NonExistentType): Integer { 42 }"#,
        ),
        (
            "Complex nesting",
            r#"def process(f: Function<(callback: Function<(x: Integer) -> String>) -> String>): String { "result" }"#,
        ),
    ];

    for (description, program) in error_test_cases {
        match parse_program(program) {
            Ok(mut parsed_program) => match typecheck_program(&mut parsed_program) {
