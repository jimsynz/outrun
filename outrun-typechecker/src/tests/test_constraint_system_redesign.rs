...
fn test_unary_minus_parameter_name_based_constraint() {
    ...
    match result {
        Ok(_compilation_result) => {
            println!("✅ UnaryMinus constraint resolution succeeded with parameter-name-based system");
        }
        Err(e) => {
            eprintln!("❌ UnaryMinus constraint resolution failed: {:?}", e);
            // This should now work with the redesigned constraint system
            panic!("Constraint system should handle UnaryMinus with parameter names, not positions");
        }
    }
}
