...
fn test_map_protocol_registration_step_by_step() {
    ...
    println!("🔍 Before registration - Map protocol exists: {}", 
...
fn test_map_protocol_reference_count_issue() {
    ...
    println!("🔍 Type registry ref count: {}", std::rc::Rc::strong_count(&engine.type_registry_rc()));
    ...
    match engine.register_protocols_and_structs(&mut program) {
        Ok(_) => {
            ...
            let type_registry = engine.type_registry_rc();
            println!("🔍 Map protocol exists after registration: {}", type_registry.is_protocol("Map"));
            ...
            // Check again
            let type_registry = engine.type_registry_rc();
            println!("🔍 Map protocol exists after dropping refs: {}", type_registry.is_protocol("Map"));
        }
        Err(e) => {
...
fn test_map_protocol_with_implementation() {
    ...
    match engine.register_protocols_and_structs(&mut program) {
        Ok(_) => {
            ...
            let type_registry = engine.type_registry_rc();
            println!("🔍 Map protocol exists: {}", type_registry.is_protocol("Map"));
            println!("🔍 Default protocol exists: {}", type_registry.is_protocol("Default"));
        }
        Err(e) => {
            println!("❌ Phase 2 failed: {}", e);
            return;
