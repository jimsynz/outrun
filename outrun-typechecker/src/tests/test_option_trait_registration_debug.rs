...
fn test_option_trait_implementation_registration() {
    ...
    println!(
        "✅ Core library compiled with {} traits, {} structs, {} implementations",
    ...
    match some_lookup {
        Some(entry) => {
            println!(
                "  ✅ Found some? on Outrun.Option.Some: {:?}",
    ...
    match none_lookup {
        Some(entry) => {
            println!(
                "  ✅ Found none? on Outrun.Option.None: {:?}",
                std::mem::discriminant(&entry)
            );
        }
        None => {
    ...
    match cross_lookup {
        Some(entry) => {
            println!(
                "  ✅ Found some? on Outrun.Option.None: {:?}",
                std::mem::discriminant(&entry)
            );
        }
        None => {
    ...
    for (module_key, module) in modules.iter() {
        match module_key {
            crate::compilation::compiler_environment::ModuleKey::TraitImpl(
                ...
                if trait_type.to_string_representation().contains("Option")
                    || impl_type.to_string_representation().contains("Option")
                {
                    println!("    Functions: {}", module.functions_by_name.len());
                    for (function_atom, _) in &module.functions_by_name {
                        let function_name = compiler_env
                            .resolve_atom_name(function_atom)
    ...
    println!(
        "\n📊 Total trait implementations found: {}",
        trait_impl_count
    );
    ...
    match smt_lookup {
        Some(entry) => {
            println!(
                "  ✅ SMT found some? on Outrun.Option.Some: {:?}",
                std::mem::discriminant(&entry)
            );
        }
        None => {
