...
fn test_intrinsic_explicit_implementation_conflict() {
    ...
    println!("  BinaryAddition for Outrun.Core.Integer64 registered: {}", 
    ...
    let integer64_file = programs.iter()
        .find(|(path, _)| path.ends_with("outrun/core/integer64.outrun"))
        .expect("Should find integer64.outrun");
    
    let mut integer64_program = parse_program(&integer64_file.1)
        .expect("Should parse integer64.outrun");
    ...
    engine.register_protocols_and_structs(&integer64_program)
        .expect("Phase 2 should succeed");
    
    let registry_after_phase2 = engine.get_protocol_registry();
    println!("  After Phase 2 - BinaryAddition for Outrun.Core.Integer64: {}", 
    ...
    engine.register_automatic_implementations(&integer64_program)
        .expect("Phase 2.5 should succeed");
        
    let registry_after_phase25 = engine.get_protocol_registry();
    println!("  After Phase 2.5 - BinaryAddition for Outrun.Core.Integer64: {}", 
    ...
    match engine.register_implementations(&integer64_program) {
        Ok(()) => {
            ...
            println!("  After Phase 3 - BinaryAddition for Outrun.Core.Integer64: {}", 
                ...
                crate::error::TypecheckError::ImplementationError(impl_err) => {
                    match impl_err {
                        crate::error::ImplementationError::ConflictingImplementation { protocol_name, type_name, .. } => {
                            println!("  🚨 CONFLICT: {} for {} during Phase 3!", protocol_name, type_name);
    ...
    engine.register_functions(&integer64_program).expect("Phase 4 should succeed");
        ...
        Err(e) => {
            ...
            match &e {
                crate::error::TypecheckError::ImplementationError(impl_err) => {
                    match impl_err {
                        crate::error::ImplementationError::ConflictingImplementation { protocol_name, type_name, .. } => {
                            println!("  🚨 CONFLICT: {} for {} during Phase 6!", protocol_name, type_name);
