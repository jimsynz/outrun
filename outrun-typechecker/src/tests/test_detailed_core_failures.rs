...
fn test_detailed_remaining_core_failures() {
    if let Ok(core_path) = default_core_library_path() {
        ...
        if collect_outrun_files(&lib_path, &mut programs).is_ok() {
                        ...
                        match &e {
                            crate::error::TypecheckError::InferenceError(inference_err) => {
                                match inference_err {
                                    ...
                                    crate::error::InferenceError::UndefinedType {
                                        type_name,
                                        ..
                                    } => {
                                        println!("      🔍 Undefined type: {}", type_name);
                                    }
                                    crate::error::InferenceError::UndefinedProtocol {
                                        protocol_name,
                                        ..
                                    } => {
                            ...
                            crate::error::TypecheckError::UnificationError(unification_err) => {
                                match unification_err {
                                    crate::error::UnificationError::TypeMismatch {
                                        expected,
                                        found,
                                        ..
                                    } => {
                                    ...
                                    crate::error::UnificationError::OccursCheckViolation {
                                        var_name,
                                        containing_type,
                                        ..
                                    } => {
                            ...
                            crate::error::TypecheckError::DispatchError(dispatch_err) => {
                                match dispatch_err {
                                    ...
                                    crate::error::DispatchError::AmbiguousDispatch {
                                        protocol_name,
                                        candidates,
                                        ..
                                    } => {
                                        println!(
                                            "      🔍 Ambiguous dispatch: {} with {} candidates",
                                            protocol_name,
                                            candidates.len()
                                        );
                                    }
                            ...
                            crate::error::TypecheckError::ConstraintError(constraint_err) => {
                                match constraint_err {
                                    crate::error::ConstraintError::MissingImplementation {
                                        protocol_name,
                                        type_name,
                                        ..
                                    } => {
            ...
            println!("\n📊 FINAL SUMMARY:");
            println!(
                "  ✅ Successfully type checked: {} files ({:.1}%)",
                successful_files.len(),
                (successful_files.len() as f64
                    / (successful_files.len() + failed_files.len()) as f64)
                    * 100.0
            );
            println!(
                "  ❌ Failed to type check: {} files ({:.1}%)",
                failed_files.len(),
                (failed_files.len() as f64 / (successful_files.len() + failed_files.len()) as f64)
                    * 100.0
            );
