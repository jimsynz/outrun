...
use crate::{TypeInferenceEngine, types::{Type, ProtocolId, TypeId, ModuleId}};
...
fn test_boolean_protocol_concrete_compatibility() {
    ...
    engine.protocol_registry_mut().add_local_module(ModuleId::new("Boolean"));
    engine.protocol_registry_mut().add_local_module(ModuleId::new("Outrun.Core.Boolean"));
    engine.protocol_registry_mut().add_local_module(ModuleId::new("TestModule"));
    ...
    engine.protocol_registry_mut().register_implementation(
    ...
    assert!(result, "Outrun.Core.Boolean should be compatible with Boolean protocol");
}
...
fn test_string_contains_scenario() {
    ...
    // Add modules as local to avoid orphan rule violations
    engine.protocol_registry_mut().add_local_module(ModuleId::new("Boolean"));
    engine.protocol_registry_mut().add_local_module(ModuleId::new("Option"));
    engine.protocol_registry_mut().add_local_module(ModuleId::new("Outrun.Core.Boolean"));
    engine.protocol_registry_mut().add_local_module(ModuleId::new("TestModule"));
