...
use crate::{typecheck_program_collection, typecheck_with_core_library, ProgramCollection};
use outrun_parser::{
...
fn test_typecheck_program_collection_multiple_files() {
    ...
    collection.add_program(
        "file1.outrun".to_string(),
        program1,
        "def func1(): Integer { 1 }".to_string(),
    );
    collection.add_program(
        "file2.outrun".to_string(),
        program2,
...
fn test_typecheck_with_core_library_simple() {
    ...
    // Should have core library protocols + user function
    assert!(!result.compilation_order.is_empty());
    assert!(!result.protocols.is_empty()); // Core library should provide protocols
...
fn test_error_reporting_with_multiple_files() {
    ...
    collection.add_program(
        "good1.outrun".to_string(),
        program1,
        "def good_func1(): Integer { 1 }".to_string(),
    );
    collection.add_program(
        "good2.outrun".to_string(),
        program2,
        "def good_func2(): Integer { 2 }".to_string(),
    );
...
fn test_compilation_order_consistency() {
    ...
    collection.add_program(
        "z_last.outrun".to_string(),
        program3,
        "def third(): Integer { 3 }".to_string(),
    );
    collection.add_program(
        "a_first.outrun".to_string(),
        program1,
        "def first(): Integer { 1 }".to_string(),
    );
    collection.add_program(
        "m_middle.outrun".to_string(),
        program2,
        "def second(): Integer { 2 }".to_string(),
    );
    ...
    assert!(result
        .compilation_order
        .contains(&"a_first.outrun".to_string()));
    assert!(result
        .compilation_order
        .contains(&"m_middle.outrun".to_string()));
    assert!(result
        .compilation_order
        .contains(&"z_last.outrun".to_string()));
}
