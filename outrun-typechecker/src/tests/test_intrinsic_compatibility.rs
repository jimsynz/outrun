...
mod intrinsic_compatibility_tests {
    ...
    use crate::types::{Type, ProtocolId, TypeId, SelfBindingContext};
    ...
    fn test_self_type_resolution() {
        ...
        assert!(resolved.is_some(), "SelfType should resolve in Implementation context");
        
        let resolved_type = resolved.unwrap();
        match resolved_type {
            Type::Concrete { id, .. } => {
                assert_eq!(id.name(), "Outrun.Core.List", "Should resolve to Outrun.Core.List");
            }
    ...
    fn test_any_protocol_compatibility() {
        ...
        engine.register_protocols_and_structs(&program).expect("Phase 2 should succeed"); engine.register_automatic_implementations(&program).expect("Phase 2.5 should succeed"); engine.register_implementations(&program).expect("Phase 3 should succeed"); engine.register_functions(&program).expect("Phase 4 should succeed");
        engine.register_implementations(&program).expect("Implementation registration should succeed");
        ...
        assert!(is_compatible, "TestStruct should be compatible with Any protocol via automatic implementation");
    }
    ...
    fn test_self_to_any_compatibility() {
        ...
        println!("Compatibility result: any_protocol -> self_type = {}", is_compatible_1);
        
        let is_compatible_2 = engine.test_types_are_compatible(&self_type, &any_protocol);
        println!("Compatibility result: self_type -> any_protocol = {}", is_compatible_2);
        
        // Also test the reverse (which is what should happen after self resolution)
        if let Some(resolved_self) = self_type.resolve_self() {
            let forward_compatible = engine.test_types_are_compatible(&any_protocol, &resolved_self);
            println!("Compatibility result: any_protocol -> resolved_self = {}", forward_compatible);
            
            let reverse_compatible = engine.test_types_are_compatible(&resolved_self, &any_protocol);
            println!("Compatibility result: resolved_self -> any_protocol = {}", reverse_compatible);
            
            // Check if Outrun.Core.List implements Any
            if let Type::Concrete { id, .. } = &resolved_self {
                let implements_any = engine.get_protocol_registry().type_satisfies_protocol(id, &ProtocolId::new("Any"));
                println!("Outrun.Core.List implements Any: {}", implements_any);
            }
        }
        
        assert!(is_compatible_2, "Self type should be compatible with Any protocol (Outrun.Core.List)");
    }
}