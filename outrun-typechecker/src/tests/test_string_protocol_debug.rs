...
use crate::types::{Type, TypeId, ProtocolId};
...
"#;
    ...
    let protocol_program = parse_program(string_protocol_code).expect("Failed to parse String protocol");
    
    // Collect protocol definitions
    engine.register_protocols_and_structs(&protocol_program).expect("Phase 2 should succeed");
    engine.register_implementations(&protocol_program).expect("Phase 3 should succeed");
    engine.register_functions(&protocol_program).expect("Phase 4 should succeed");
...
"#;
    ...
    let impl_program = parse_program(string_impl_code).expect("Failed to parse String implementation");
    
    // Collect struct definitions first
    engine.register_protocols_and_structs(&impl_program).expect("Phase 2 should succeed");
    
    // Register automatic implementations
    engine.register_automatic_implementations(&impl_program).expect("Phase 2.5 should succeed");
    
    // Register explicit implementations
    engine.register_implementations(&impl_program).expect("Failed to register String implementation");
    ...
    println!("Total implementations in registry: {}", implementation_count);
    ...
    let string_impls_len = {
        ...
        for impl_info in &string_impls {
            println!("  - {} implements String", impl_info.implementing_type.name());
        }
        string_impls.len()
    };
    ...
    let type_impls_len = {
        ...
        println!("Protocols implemented by Outrun.Core.String: {}", type_impls.len());
        for impl_info in &type_impls {
            println!("  - Outrun.Core.String implements {}", impl_info.protocol_id.0);
        }
        type_impls.len()
    };
...
"#;
    ...
    let mut test_program = parse_program(test_function_code).expect("Failed to parse test function");
    ...
    // Assertions
    assert!(has_direct_impl, "Outrun.Core.String should directly implement String protocol");
    assert!(satisfies_protocol, "Outrun.Core.String should satisfy String protocol");
    assert!(string_impls_len > 0, "String protocol should have implementations");
    assert!(type_impls_len > 0, "Outrun.Core.String should implement protocols");
    assert!(compatible, "Outrun.Core.String should be compatible with String protocol");
}
