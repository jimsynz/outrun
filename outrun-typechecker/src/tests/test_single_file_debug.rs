...
fn test_single_option_file_debug() {
    ...
    let boolean_code = fs::read_to_string("/Users/jmshrtn/Dev/harton.dev/outrun/outrun/outrun-core/lib/boolean.outrun")
    ...
    let has_boolean = engine.get_protocol_registry().has_protocol(&boolean_protocol_id);
    println!("Boolean protocol registered: {}", has_boolean);
    
    // Now try to process option.outrun
    let option_code = fs::read_to_string("/Users/jmshrtn/Dev/harton.dev/outrun/outrun/outrun-core/lib/option.outrun")
        .expect("Should read option.outrun");
    ...
    match result {
        Ok(()) => {
            ...
            let dispatcher = crate::dispatch::FunctionDispatcher::new(&engine.get_protocol_registry(), &engine.function_registry());
            match dispatcher.resolve_qualified_call("Option.some?", None, None) {
                Ok(dispatch_result) => {
                    match dispatch_result {
                        crate::dispatch::DispatchResult::Resolved(resolved) => {
                            println!("Option.some? return type: {:?}", resolved.function_info.return_type);
