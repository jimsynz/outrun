...
#[test]
fn test_empty_collections_typed_ast() {
    // Test only tuple for now since empty lists/maps need type hints and let bindings aren't implemented
    let source = "()";
    ...
    match &typed_program.items[0].kind {
        crate::checker::TypedItemKind::Expression(expr) => match &expr.kind {
            TypedExpressionKind::Tuple { elements, .. } => {
                assert!(elements.is_empty(), "Empty tuple should have no elements");
            }
