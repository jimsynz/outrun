...
"#;
    ...
    engine.register_protocols_and_structs(&boolean_program).expect("Boolean registration should succeed");
    
    println!("Processing option.outrun...");
    engine.register_protocols_and_structs(&option_program).expect("Option registration should succeed");
    
    println!("Processing string.outrun...");
    engine.register_protocols_and_structs(&string_program).expect("String registration should succeed");
    ...
    println!("Boolean protocol registered: {}", engine.get_protocol_registry().has_protocol(&boolean_protocol_id));
    println!("Option protocol registered: {}", engine.get_protocol_registry().has_protocol(&option_protocol_id));
    println!("String protocol registered: {}", engine.get_protocol_registry().has_protocol(&string_protocol_id));
    
    println!("=== PHASE 2.5: Register automatic implementations ===");
    engine.register_automatic_implementations(&boolean_program).expect("Boolean auto-impl should succeed");
    engine.register_automatic_implementations(&option_program).expect("Option auto-impl should succeed");
    engine.register_automatic_implementations(&string_program).expect("String auto-impl should succeed");
    
    println!("=== PHASE 3: Register explicit implementations ===");
    engine.register_implementations(&boolean_program).expect("Boolean impl should succeed");
    engine.register_implementations(&option_program).expect("Option impl should succeed");
    engine.register_implementations(&string_program).expect("String impl should succeed");
    ...
    let success3 = match &result3 {
        ...
        Err(e) => {
            ...
            if error_str.contains("Concrete { id: TypeId(\"Boolean\")") {
                ...
                println!("This means String.contains? return type 'Boolean' was treated as concrete");
            }
            false
        }
    };
