...
fn test_constraint_expression_extraction_simple() {
    ...
    assert!(
        ...
        "display function should be registered in impl scope"
    );
    let function_info = function_info.unwrap();

    // Should have both T (from constraint) and U (from Wrapper<U>) as generic parameters
    assert!(
        !function_info.generic_parameters.is_empty(),
        "Function should have generic parameters"
    );
    assert!(
        function_info.generic_parameters.contains(&"T".to_string()),
        "Function should have T as generic parameter from constraint: {:?}",
        function_info.generic_parameters
    );
    assert!(
        function_info.generic_parameters.contains(&"U".to_string()),
        "Function should have U as generic parameter from type spec: {:?}",
        function_info.generic_parameters
    );
    assert!(
        function_info.is_generic,
        "Function should be marked as generic"
    );
}
...
fn test_constraint_expression_extraction_complex() {
    ...
    let program_text = r#"
        ...
        struct Adapter<C, D>(input: C, output: D)
    ...
    assert!(
        function_info.is_some(),
        "convert function should be registered in impl scope"
    );
    let function_info = function_info.unwrap();
    ...
    assert!(
        function_info.generic_parameters.contains(&"A".to_string()),
        "Function should have A as generic parameter: {:?}",
        function_info.generic_parameters
    );
    assert!(
        function_info.generic_parameters.contains(&"B".to_string()),
        "Function should have B as generic parameter: {:?}",
        function_info.generic_parameters
    );
    assert!(
        function_info.generic_parameters.contains(&"C".to_string()),
        "Function should have C as generic parameter: {:?}",
        function_info.generic_parameters
    );
    assert!(
        function_info.generic_parameters.contains(&"D".to_string()),
        "Function should have D as generic parameter: {:?}",
        function_info.generic_parameters
    );
    assert!(
        function_info.is_generic,
...
fn test_constraint_expression_extraction_nested() {
    ...
    assert!(
        function_info.is_some(),
        "transform function should be registered in impl scope"
    );
    let function_info = function_info.unwrap();
    ...
    assert!(
        function_info.generic_parameters.contains(&"T".to_string()),
        "Function should have T as generic parameter: {:?}",
        function_info.generic_parameters
    );
    assert!(
        function_info.generic_parameters.contains(&"U".to_string()),
        "Function should have U as generic parameter: {:?}",
        function_info.generic_parameters
    );
    assert!(
        function_info.is_generic,
...
fn test_constraint_expression_no_duplicates() {
    ...
    assert_eq!(
        t_count, 1,
        "T should appear exactly once in generic parameters: {:?}",
        function_info.generic_parameters
    );
    assert!(
        function_info.generic_parameters.contains(&"T".to_string()),
