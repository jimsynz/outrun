...
"#;
    ...
    let boolean_program = parse_program(boolean_protocol_code).expect("Parse Boolean should succeed");
    ...
    engine.register_protocols_and_structs(&boolean_program).expect("Boolean protocol registration should succeed");
    
    // Check if Boolean is now recognized as a protocol
    let boolean_protocol_id = crate::types::ProtocolId::new("Boolean");
    let has_boolean_protocol = engine.get_protocol_registry().has_protocol(&boolean_protocol_id);
    ...
    engine.register_protocols_and_structs(&option_program).expect("Option protocol registration should succeed");
