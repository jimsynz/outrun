...
fn test_string_interpolation_with_non_display_type() {
    ...
    match result.unwrap_err() {
        TypeError::StringInterpolationDisplayError { type_name, .. } => {
            assert_eq!(type_name, "CustomType");
        }
        _ => panic!("Expected StringInterpolationDisplayError"),
    }
}
...
fn test_string_interpolation_no_display_trait_defined() {
    ...
    // Should get error because Display trait doesn't exist
    match result.unwrap_err() {
        TypeError::StringInterpolationDisplayError { type_name, .. } => {
