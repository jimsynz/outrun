...
fn test_phase3_registration_detailed_tracking() {
    ...
    for (file_path, program) in &parsed_programs {
        ...
        if is_problematic {
            ...
            let impl_count = program.items.iter().filter(|item| {
                matches!(item.kind, outrun_parser::ItemKind::ImplBlock(_))
            }).count();
            ...
            for item in &program.items {
                if let outrun_parser::ItemKind::ImplBlock(impl_block) = &item.kind {
                    let protocol_name = impl_block.protocol_spec.path
                        ...
                        .collect::<Vec<_>>()
                        .join(".");
                    let type_name = impl_block.type_spec.path
                        .iter()
                        .map(|segment| segment.name.as_str())
                        .collect::<Vec<_>>()
                        .join(".");
                        ...
                        crate::error::TypecheckError::ImplementationError(impl_err) => {
                            match impl_err {
                                crate::error::ImplementationError::ConflictingImplementation { protocol_name, type_name, .. } => {
                                    println!("   🚨 Conflicting implementation: {} for {}", protocol_name, type_name);
                                }
                                _ => {
