...
fn test_anonymous_function_parsing_available() {
    ...
    let inputs = vec![r#"fn { -> 42 }"#, r#"fn { x: Integer -> x }"#];
        ...
        match parse_expression(input) {
            Ok(expr) => match &expr.kind {
                ExpressionKind::AnonymousFunction(anon_fn) => {
                    assert!(
                        !anon_fn.clauses.is_empty(),
                        "Anonymous function should have clauses"
                    );
                    println!("Successfully parsed: {}", input);
                }
                other => {
                    println!(
                        "Parsed as different expression type for '{}': {:?}",
                        input, other
                    );
                }
            },
            Err(parse_error) => {
                println!(
                    "Parse error for '{}' (might be expected): {:?}",
                    input, parse_error
                );
            }
        }
...
fn test_function_type_display() {
    ...
    assert!(
        ...
        "Display should contain 'Function'"
    );
    assert!(
        display_string.contains("x: Integer"),
        "Display should show parameter"
    );
    assert!(
        display_string.contains("String"),
        "Display should show return type"
    );
}
...
fn test_integration_function_inference() {
    ...
    for (input, description) in test_cases {
        let expr =
            parse_expression(input).unwrap_or_else(|_| panic!("Should parse {}", description));
        let mut expr_mut = expr.clone();
        let result = engine.infer_expression(&mut expr_mut, &mut context);
