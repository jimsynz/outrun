...
//! modules, and protocol implementations across multiple compilation units.
//!
...
use crate::compilation::visitors::{
    ImplExtractionVisitor, ProtocolExtractionVisitor, StructExtractionVisitor,
};
use crate::dependency_graph::DependencyGraph;
use crate::desugaring::DesugaringVisitor;
use crate::error::{SpanExt, TypeError};
use crate::types::protocols::ProtocolConstraint;
use crate::unification::{StructuredType, UnificationContext};
use crate::visitor::Visitor;
use outrun_parser::{
    FunctionDefinition, ImplBlock, Program, ProtocolDefinition, ProtocolFunction, StructDefinition,
};
...
pub enum ModuleKey {
    /// Module identified by its base name hash (for protocols and structs)
    Module(u64),
    /// Protocol implementation module identified by its full structured types
    ProtocolImpl(Box<StructuredType>, Box<StructuredType>),
}
...
pub enum ModuleKind {
    ...
    ProtocolImpl,
    /// Auto-implemented protocol module (generated by compiler)
    AutoImplemented,
}
...
pub enum FunctionType {
    /// Static protocol function (defs keyword)
    ProtocolStatic,
    /// Protocol function signature only (def without body)
    ProtocolSignature,
    /// Protocol function with default implementation (def with body)
    ProtocolDefault,
    /// Static function on a type (not in protocol)
    TypeStatic,
...
impl UnifiedFunctionEntry {
    ...
    pub fn definition(&self) -> &outrun_parser::FunctionDefinition {
        match self {
            UnifiedFunctionEntry::ProtocolStatic { definition, .. } => definition,
            UnifiedFunctionEntry::ProtocolSignature { definition, .. } => definition,
            UnifiedFunctionEntry::ProtocolDefault { definition, .. } => definition,
    ...
    pub fn typed_definition(&self) -> &Option<TypedFunctionDefinition> {
        match self {
            UnifiedFunctionEntry::ProtocolStatic {
                typed_definition, ..
            } => typed_definition,
            UnifiedFunctionEntry::ProtocolSignature {
                typed_definition, ..
            } => typed_definition,
            UnifiedFunctionEntry::ProtocolDefault {
                typed_definition, ..
            } => typed_definition,
    ...
    pub fn function_id(&self) -> &str {
        match self {
            UnifiedFunctionEntry::ProtocolStatic { function_id, .. } => function_id,
            UnifiedFunctionEntry::ProtocolSignature { function_id, .. } => function_id,
            UnifiedFunctionEntry::ProtocolDefault { function_id, .. } => function_id,
    ...
    pub fn is_guard(&self) -> bool {
        match self {
            UnifiedFunctionEntry::ProtocolStatic { is_guard, .. } => *is_guard,
            UnifiedFunctionEntry::ProtocolSignature { is_guard, .. } => *is_guard,
            UnifiedFunctionEntry::ProtocolDefault { is_guard, .. } => *is_guard,
    ...
    pub fn function_type(&self) -> FunctionType {
        match self {
            UnifiedFunctionEntry::ProtocolStatic { .. } => FunctionType::ProtocolStatic,
            UnifiedFunctionEntry::ProtocolSignature { .. } => FunctionType::ProtocolSignature,
            UnifiedFunctionEntry::ProtocolDefault { .. } => FunctionType::ProtocolDefault,
            UnifiedFunctionEntry::TypeStatic { .. } => FunctionType::TypeStatic,
            UnifiedFunctionEntry::ImplFunction { .. } => FunctionType::ImplFunction,
            UnifiedFunctionEntry::Intrinsic { .. } => FunctionType::ProtocolStatic, // Treat intrinsics as static for now
        }
    }
...
#[derive(Debug, Clone)]
pub struct Module {
    /// The kind of module (struct, protocol, or protocol impl)
    ...
    /// Protocol constraints for protocol modules (empty for struct/impl modules)
...
impl Module {
    ...
    /// Set protocol constraints for this module (only valid for protocol modules)
    pub fn set_protocol_constraints(&mut self, constraints: Vec<ProtocolConstraint>) {
        self.protocol_constraints = constraints;
    }

    /// Get protocol constraints for this module
    pub fn get_protocol_constraints(&self) -> &[ProtocolConstraint] {
...
pub struct CompilerEnvironment {
    ...
    protocols: Arc<RwLock<HashMap<TypeNameId, ProtocolDefinition>>>,
}
...
struct CompilationState {
    ...
    /// Dispatch table for runtime protocol method resolution
...
impl CompilerEnvironment {
    ...
    fn internal_compile_collection(
        ...
        // Step 2: Phase 1 - Extract all protocols
        let protocols = self.extract_protocols(collection, &compilation_order)?;

        // Store protocols in CompilerEnvironment
        for (type_id, protocol_def) in &protocols {
            self.add_protocol(type_id.clone(), protocol_def.clone());
        }

        // Create modules for protocols (similar to struct modules)
        for (type_id, protocol_def) in &protocols {
            // Create the structured type for this protocol
            let structured_type = if let Some(generic_params) = &protocol_def.generic_params {
                // Generic protocol like Map<K, V>
                ...
                // Simple protocol without generics
                StructuredType::Simple(type_id.clone())
            };

            // Create module for the protocol
            let module_key = ModuleKey::Module(type_id.hash);
            self.get_or_create_module(
                module_key,
                ModuleKind::Protocol,
                SourceLocation::File("protocol_definition".to_string()),
                structured_type,
            );
        }
        ...
        // Create modules for structs (similar to how protocol modules are created)
        for (type_id, struct_def) in &structs {
        ...
        // Step 3.5: Generate auto-implementations for protocols that support it
        ...
        self.extract_functions(collection, &compilation_order)?;

        // Step 6: Phase 5 - Desugar all programs (transform operators to protocol function calls)
        let desugared_collection = self.desugar_programs(collection, &compilation_order)?;

        // Step 7: Phase 6 - Type check everything
        match self.type_check_all(
            &desugared_collection,
            &compilation_order,
            &protocols,
            &structs,
            &implementations,
        ...
        // Step 8: Phase 7 - Build comprehensive typed AST
        let typed_programs =
            self.build_typed_ast(&desugared_collection, &compilation_order, &structs)?;
    ...
    fn resolve_dependencies(
        ...
        let result = dependency_graph.resolve_with_protocol_cycles_allowed();
    ...
    /// Extract protocol definitions (Phase 1)
    fn extract_protocols(
        &mut self,
        collection: &ProgramCollection,
        order: &[String],
    ) -> Result<HashMap<TypeNameId, ProtocolDefinition>, Vec<TypeError>> {
        let mut visitor = ProtocolExtractionVisitor::default();

        for file_path in order {
            if let Some(program) = collection.programs.get(file_path) {
                if let Err(err) =
                    <ProtocolExtractionVisitor as Visitor<()>>::visit_program(&mut visitor, program)
                {
        ...
        for (name, protocol_def) in visitor.protocols {
            let type_name_id = self.intern_type_name(&name);
            protocols.insert(type_name_id, protocol_def);
        }
    ...
    /// Generate automatic protocol implementations for types that don't have manual ones
    ...
    fn generate_auto_implementations(
        ...
        // Auto-implement Inspect protocol for all concrete types
        let inspect_protocol_id = self.intern_type_name("Inspect");
        let inspect_protocol_type = StructuredType::Simple(inspect_protocol_id);

        for struct_type_id in structs.keys() {
            let struct_type = StructuredType::Simple(struct_type_id.clone());

            // Auto-implement Inspect protocol for this struct
            // Since Inspect has a default implementation, we only need to register
            // the protocol implementation - no need to create function entries
            self.register_protocol_implementation(
                struct_type.clone(),
                inspect_protocol_type.clone(),
            );
        }

        // Auto-implement Inspect protocol for built-in generic types
        self.generate_builtin_inspect_implementations(&inspect_protocol_type)?;
    ...
    fn generate_builtin_inspect_implementations(
        &self,
        inspect_protocol_type: &StructuredType,
    ) -> Result<(), Vec<TypeError>> {
                    ...
                    if let Err(err) = self.extract_function_from_item(item, file_path) {
                        ...
                        match &item.kind {
                            ...
                            outrun_parser::ItemKind::ProtocolDefinition(protocol_def) => {
                                let func_count = protocol_def.functions.len();
                                _total_functions += func_count;
                                // Extracted {func_count} protocol functions from {protocol_def.name_as_string()}
                            }
    ...
    fn extract_function_from_item(
        ...
        match &item.kind {
            ...
            ItemKind::ProtocolDefinition(protocol_def) => {
                // Extract functions from protocol definition
                for protocol_func in &protocol_def.functions {
                    self.extract_protocol_function(protocol_func, protocol_def, source_file)?;
                }
                Ok(())
            }
            ItemKind::ImplBlock(impl_block) => {
                // Extract protocol implementations (no "methods" in functional language)
                // All functions in impl blocks are protocol function implementations
                for func_def in &impl_block.methods {
                    self.extract_protocol_impl_function(func_def, impl_block, source_file)?;
                }
                Ok(())
            }
    ...
    /// Extract a protocol function and add it to the protocol module
    fn extract_protocol_function(
        &self,
        protocol_func: &ProtocolFunction,
        protocol_def: &outrun_parser::ProtocolDefinition,
        source_file: &str,
    ) -> Result<(), TypeError> {
        // Get the function definition from the ProtocolFunction enum
        // Handle all types of protocol functions
        let (func_def, function_type) = match protocol_func {
            ProtocolFunction::Definition(def) => (def.clone(), FunctionType::ProtocolDefault),
            ProtocolFunction::Signature(sig) => {
                ...
                (func_def, FunctionType::ProtocolSignature)
            }
            ProtocolFunction::StaticDefinition(static_def) => {
                ...
                (func_def, FunctionType::ProtocolStatic)
            }
        };

        // Create function entry
        let protocol_name = protocol_def.name_as_string();
        let function_id = format!("protocol::{}::{}", protocol_name, func_def.name.name);
        let is_guard = func_def.name.name.ends_with('?');
        let entry = match function_type {
            FunctionType::ProtocolStatic => UnifiedFunctionEntry::ProtocolStatic {
                definition: func_def.clone(),
                typed_definition: None,
                function_id,
                is_guard,
            },
            FunctionType::ProtocolSignature => UnifiedFunctionEntry::ProtocolSignature {
                definition: func_def.clone(),
                typed_definition: None,
                function_id,
                is_guard,
            },
            FunctionType::ProtocolDefault => UnifiedFunctionEntry::ProtocolDefault {
                definition: func_def.clone(),
                typed_definition: None,
                function_id,
                is_guard,
            },
            _ => panic!("Unexpected function type for protocol function: {function_type:?}"),
        };

        // Create protocol module if it doesn't exist
        let protocol_type_id = self.intern_type_name(&protocol_name);
        let module_key = ModuleKey::Module(protocol_type_id.hash);
        ...
        // Add function to protocol module
        let function_name = self.intern_atom_name(&func_def.name.name);
        // Adding protocol function to protocol
        self.add_function_to_module(module_key, protocol_type, function_name, entry);

        Ok(())
    }

    /// Extract a protocol implementation function and add it to the impl module
    /// In Outrun, impl blocks provide implementations of protocol functions for specific types
    fn extract_protocol_impl_function(
        &self,
        func_def: &FunctionDefinition,
        ...
        // Create function entry for protocol implementation
        let function_id = format!(
            "impl::{}::for::{}::{}",
            protocol_name, impl_type_name, func_def.name.name
        );
        let is_guard = func_def.name.name.ends_with('?');
        ...
        // IMPORTANT: Always use Simple protocol type for registrations, even for generic protocols
        // This matches the lookup logic in function dispatch which also uses Simple protocol types
        ...
        let module_key =
            ModuleKey::ProtocolImpl(Box::new(protocol_type.clone()), Box::new(impl_type.clone()));

        // Create protocol implementation module if it doesn't exist
        self.get_or_create_module(
            module_key.clone(),
            ModuleKind::ProtocolImpl,
            SourceLocation::File(source_file.to_string()),
            impl_type.clone(),
        );

        // Add protocol function implementation to the module
        let function_name = self.intern_atom_name(&func_def.name.name);
    ...
    /// Desugar all programs (Phase 5) - Transform operators to protocol function calls
    fn desugar_programs(
        &mut self,
        collection: &ProgramCollection,
        order: &[String],
    ...
    fn type_check_all(
        ...
        collection: &ProgramCollection,
        order: &[String],
        _protocols: &HashMap<TypeNameId, ProtocolDefinition>,
        _structs: &HashMap<TypeNameId, StructDefinition>,
        implementations: &[ImplBlock],
        external_variables: HashMap<String, StructuredType>,
    ) -> Result<(), Vec<TypeError>> {
        // Register protocol implementations with the unification context
        self.register_protocol_implementations(implementations)?;
    ...
    /// Register protocol implementations with the unification context
    fn register_protocol_implementations(
        &mut self,
        implementations: &[ImplBlock],
    ) -> Result<(), Vec<TypeError>> {
        ...
        for impl_block in implementations {
            ...
            // Get protocol name from TypeSpec path
            let protocol_name = impl_block
                .protocol_spec
                .path
                .iter()
            ...
            // not the generic type (e.g., "List<T>")
            let protocol_type_id = self.intern_type_name(&protocol_name);
            let protocol_structured = StructuredType::Simple(protocol_type_id);
            ...
            self.register_protocol_implementation(impl_structured, protocol_structured);
        }
    ...
    fn process_registered_impl_functions(
        ...
        for (module_key, module) in modules.iter() {
            // Only process protocol implementation modules
            if let ModuleKey::ProtocolImpl(_protocol_type, _impl_type) = module_key {
                for function_entry in module.functions_by_name.values() {
                    ...
                    } = function_entry
                    {
                        if typed_definition.is_none() {
                            ...
                            // This ensures that function updates happen in the right protocol implementation module
    ...
    /// Get a protocol implementation module
    pub fn get_protocol_impl(
        &self,
        protocol_type: StructuredType,
        struct_type: StructuredType,
    ) -> Option<Module> {
        let key = ModuleKey::ProtocolImpl(Box::new(protocol_type), Box::new(struct_type));
        self.modules.read().unwrap().get(&key).cloned()
    }
    ...
    /// Add a protocol implementation module
    pub fn add_protocol_impl(
        &self,
        protocol_type: StructuredType,
        struct_type: StructuredType,
        module: Module,
    ...
    // These methods replace the ProtocolRegistry functionality
    ...
    pub fn implements_protocol(
        ...
        self.find_generic_protocol_implementation(impl_type, protocol_type)
    }

    /// Find a generic protocol implementation that can be instantiated for concrete types
    fn find_generic_protocol_implementation(
        &self,
        concrete_impl_type: &StructuredType,
        concrete_protocol_type: &StructuredType,
    ) -> bool {
        let modules = self.modules.read().unwrap();

        // Search through all protocol implementation modules
        for (module_key, _module) in modules.iter() {
            if let ModuleKey::ProtocolImpl(protocol_type, impl_type) = module_key {
                // Check if this is a generic implementation that could match our concrete types
                if self.generic_types_match(protocol_type, concrete_protocol_type)
    ...
    fn generic_types_match(
        ...
        match (generic_type, concrete_type) {
            ...
            // Simple protocol type can be instantiated to a generic type with the same base
    ...
    /// Get protocol constraints for a protocol type
    pub fn get_protocol_constraints(
        &self,
        protocol_type: &StructuredType,
    ) -> Vec<ProtocolConstraint> {
        if let StructuredType::Simple(type_name_id) = protocol_type {
            let module_key = ModuleKey::Module(type_name_id.hash);
            if let Some(module) = self.modules.read().unwrap().get(&module_key) {
                if matches!(module.module_kind, ModuleKind::Protocol) {
                    return module.protocol_constraints.clone();
                }
            }
        }
        Vec::new()
    ...
    /// Get all types that implement a specific protocol
    pub fn get_protocol_implementations(
        ...
        for (module_key, _module) in modules.iter() {
            if let ModuleKey::ProtocolImpl(module_protocol_type, impl_type) = module_key {
                // Use StructuredType equality for protocol matching
                if **module_protocol_type == *protocol_type {
    ...
    /// Check if a type is a protocol (has a protocol module)
    pub fn is_protocol(&self, type_ref: &StructuredType) -> bool {
        if let StructuredType::Simple(type_name_id) = type_ref {
            let module_key = ModuleKey::Module(type_name_id.hash);
            if let Some(module) = self.modules.read().unwrap().get(&module_key) {
                return matches!(module.module_kind, ModuleKind::Protocol);
            }
        }
        false
    }

    /// Register a protocol (creates a Protocol module)
    pub fn register_protocol(&self, protocol_type: StructuredType) {
        if let StructuredType::Simple(protocol_type_id) = &protocol_type {
            let module_key = ModuleKey::Module(protocol_type_id.hash);
    ...
    /// Register a protocol implementation (creates a ProtocolImpl module)
    pub fn register_protocol_implementation(
        &self,
        impl_type: StructuredType,
        protocol_type: StructuredType,
    ) {
        // First ensure the protocol itself is registered
        self.register_protocol(protocol_type.clone());
    ...
    /// Register a protocol definition with the environment
    /// This replaces ProtocolRegistry::register_protocol
    pub fn register_protocol_definition(&self, protocol_def: outrun_parser::ProtocolDefinition) {
        let protocol_name = protocol_def.name_as_string();
        let protocol_type_id = self.intern_type_name(&protocol_name);
        let protocol_type = StructuredType::Simple(protocol_type_id.clone());

        // Create or get the protocol module
        let module_key = ModuleKey::Module(protocol_type_id.hash);
        self.get_or_create_module(
            module_key.clone(),
            ModuleKind::Protocol,
        ...
        // Add protocol constraints to the module if any
        if let Ok(mut modules) = self.modules.write() {
            if let Some(module) = modules.get_mut(&module_key) {
                // Convert protocol constraints to ProtocolConstraint format
                // This would need to be implemented based on the protocol definition structure
                // For now, we'll set empty constraints
                module.set_protocol_constraints(Vec::new());
            }
        }

        // Extract and register protocol functions
        for protocol_function in &protocol_def.functions {
            let function_name_str = match protocol_function {
                outrun_parser::ProtocolFunction::Signature(sig) => &sig.name.name,
                outrun_parser::ProtocolFunction::Definition(def) => &def.name.name,
            ...
            let function_entry =
                self.extract_protocol_function_entry(protocol_function, &protocol_name);
    ...
    /// Extract a UnifiedFunctionEntry from a protocol function
    /// This is a helper method for protocol definition registration
    fn extract_protocol_function_entry(
        &self,
        protocol_function: &outrun_parser::ProtocolFunction,
        protocol_name: &str,
    ) -> Option<UnifiedFunctionEntry> {
        match protocol_function {
            outrun_parser::ProtocolFunction::Definition(func_def) => {
                let function_id = format!("protocol::{}::{}", protocol_name, func_def.name.name);
                let is_guard = func_def.name.name.ends_with('?');
                Some(UnifiedFunctionEntry::ProtocolDefault {
                    definition: func_def.clone(),
                    typed_definition: None,
                    function_id,
                    is_guard,
            ...
            outrun_parser::ProtocolFunction::Signature(func_sig) => {
                // Convert signature to function definition for storage
                ...
                let function_id = format!("protocol::{}::{}", protocol_name, func_def.name.name);
                let is_guard = func_def.name.name.ends_with('?');
                Some(UnifiedFunctionEntry::ProtocolSignature {
                    definition: func_def,
                    typed_definition: None,
                    function_id,
                    is_guard,
            ...
            outrun_parser::ProtocolFunction::StaticDefinition(static_def) => {
                ...
                let function_id = format!("protocol::{}::{}", protocol_name, func_def.name.name);
                let is_guard = func_def.name.name.ends_with('?');
                Some(UnifiedFunctionEntry::ProtocolStatic {
                    definition: func_def,
                    typed_definition: None,
                    function_id,
                    is_guard,
    ...
    /// Check protocol case exhaustiveness
    /// This replaces ProtocolRegistry::check_protocol_case_exhaustiveness
    pub fn check_protocol_case_exhaustiveness(
        &self,
        protocol_type: &StructuredType,
        covered_types: &[StructuredType],
    ) -> crate::types::protocols::ExhaustivenessResult {
        let all_implementations = self.get_protocol_implementations(protocol_type);
        ...
        if missing_types.is_empty() {
            crate::types::protocols::ExhaustivenessResult::Exhaustive
        } else {
            ...
            let missing_type_ids: Vec<TypeNameId> = missing_types
                ...
                .collect();
            crate::types::protocols::ExhaustivenessResult::MissingProtocolImplementations(
                missing_type_ids,
            )
        }
    }
    ...
    pub fn load_compilation_result(&mut self, compilation_result: CompilationResult) {
        ...
        // Load structs and protocols from the compilation result
        self.load_structs_and_protocols(&compilation_result);
    ...
    /// Look up a function implementation for protocol dispatch
    ...
    pub fn lookup_impl_function(
        ...
        // Try both exact match and base type match for module keys
        let exact_module_key =
            ModuleKey::ProtocolImpl(Box::new(protocol_type.clone()), Box::new(impl_type.clone()));

        // Also try with base types for module key lookup
        let protocol_base = self.extract_base_type_for_lookup(protocol_type);
        let impl_base = self.extract_base_type_for_lookup(impl_type);
        let base_module_key = ModuleKey::ProtocolImpl(Box::new(protocol_base), Box::new(impl_base));

        if let Ok(modules) = self.modules.read() {
            // First try exact module key match
            if let Some(impl_module) = modules.get(&exact_module_key) {
                if let Some(function) = impl_module.get_function_by_name(function_name.clone()) {
                    return Some(function.clone());
                }
            }

            // Then try base type module key match
            if exact_module_key != base_module_key {
            ...
            // If no exact match, try to find a generic implementation that can unify with the concrete type
            for (module_key, module) in modules.iter() {
                if let ModuleKey::ProtocolImpl(key_protocol_type, key_impl_type) = module_key {
                    // Check if the protocol types match
                    let unification_context = self.unification_context();
                    let protocol_unifies = crate::unification::unify_structured_types(
                        protocol_type,
                        key_protocol_type,
                        &unification_context,
                        self,
                    ...
                    if protocol_unifies.is_some() {
                        // Try to unify the implementation types
                        let impl_unifies = crate::unification::unify_structured_types(
                            impl_type,
                            key_impl_type,
                            &unification_context,
                            self,
                        ...
                        if impl_unifies.is_some() {
                            // Found a matching generic implementation
                            if let Some(generic_function) =
                                module.get_function_by_name(function_name.clone())
                            {
                                // If the generic function has no typed definition, try to create one
                                // by instantiating the generic implementation with concrete types
                                if generic_function.typed_definition().is_none() {
                                    // Extract generic parameters and create substitution map
                                    if let Some(concrete_function) =
                                        self.instantiate_generic_impl_function(generic_function)
                                    {
            ...
            // If no specific impl module, check if the protocol module has the function
            // This handles default implementations
            if let StructuredType::Simple(protocol_type_id) = protocol_type {
                if let Some(_protocol_name) = self.resolve_type(protocol_type_id.clone()) {
                    let protocol_module_key = ModuleKey::Module(protocol_type_id.hash);

                    if let Some(protocol_module) = modules.get(&protocol_module_key) {
                        if let Some(function) = protocol_module.get_function_by_name(function_name)
                        {
                            return Some(function.clone());
                        }
                    }
    ...
    /// Check if an impl block has an override for a protocol function
    ...
    /// Add a protocol function entry
    /// This replaces FunctionRegistry::add_protocol_function
    pub fn add_protocol_function(
        &self,
        protocol_type: StructuredType,
        function_name: AtomId,
        entry: UnifiedFunctionEntry,
    ...
    pub fn add_impl_function(
        ...
        self.get_or_create_module(
            ...
            ModuleKind::ProtocolImpl,
            SourceLocation::File("generated".to_string()),
            protocol_type.clone(), // Use protocol type as the structured type for the module
        );

        // Add function to the module
        self.add_function_to_module(impl_module_key, protocol_type, function_name, entry);
    }
    ...
    /// This replaces DispatchTable::register_protocol_impl
    pub fn register_protocol_impl_for_dispatch(
        &self,
        protocol_type: &StructuredType,
        impl_type: &StructuredType,
    ) -> u32 {
        ...
        if let (StructuredType::Simple(protocol_id), StructuredType::Simple(impl_id)) =
            (protocol_type, impl_type)
        {
            ...
            state
                ...
                .register_protocol_impl(
                    self.intern_type_name("ComplexProtocolType"),
    ...
    /// Look up protocol implementation in dispatch table
    /// This replaces DispatchTable::lookup_protocol_impl
    pub fn lookup_protocol_impl_dispatch(
        &self,
        protocol_type: &StructuredType,
        impl_type: &StructuredType,
    ) -> Option<u32> {
    ...
    pub fn register_static_function_for_dispatch(
        ...
        if let StructuredType::Simple(type_id) = type_ref {
            state.dispatch_table.register_static_function(
                ...
                crate::types::protocols::FunctionId(function_id),
            );
        }
    }
    ...
    // =============================================================================
    // Struct and Protocol Management Methods (Replacing TypeCheckingContext functionality)
    // =============================================================================
    ...
    pub fn add_protocol(&self, type_id: TypeNameId, protocol_def: ProtocolDefinition) {
        self.protocols
            .write()
            .unwrap()
            .insert(type_id, protocol_def);
    }
    ...
    pub fn get_protocol(&self, type_id: &TypeNameId) -> Option<ProtocolDefinition> {
        self.protocols.read().unwrap().get(type_id).cloned()
    }
    ...
    pub fn get_all_protocols(&self) -> HashMap<TypeNameId, ProtocolDefinition> {
        self.protocols.read().unwrap().clone()
    }

    /// Load structs and protocols from a compilation result
    pub fn load_structs_and_protocols(&self, compilation_result: &CompilationResult) {
        // Load structs
        {
        ...
        // Load protocols
        {
            ...
            for (type_id, protocol_def) in &compilation_result.protocols {
                protocols.insert(type_id.clone(), protocol_def.clone());
            }
        }
    }

    /// Create a compilation result that includes structs and protocols from this environment
    pub fn create_compilation_result_with_structs_and_protocols(
        &self,
        compilation_order: Vec<String>,
        implementations: Vec<ImplBlock>,
    ...
    pub fn load_from_compilation_phase_context(
        ...
        // Load structs and protocols from the type checking context
        for (type_id, struct_def) in &phase_context.type_checking_context.structs {
            self.add_struct(type_id.clone(), struct_def.clone());
        }

        for (type_id, protocol_def) in &phase_context.type_checking_context.protocols {
            self.add_protocol(type_id.clone(), protocol_def.clone());
        }
    }
