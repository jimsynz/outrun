...
# Final comment"#;
    ...
    assert_eq!(program.debug_info.comments[1].content, " Inline comment after constant");
    ...
    assert_eq!(program.debug_info.comments[3].content, " Comment before second constant  ");
    ...
    for i in 1..program.debug_info.comments.len() {
        assert!(
            program.debug_info.comments[i-1].span.start < program.debug_info.comments[i].span.start,
            "Comments should be ordered by span position"
        );
    }
    
    // Verify that comments don't appear as items anymore
    let comment_items: Vec<_> = program.items
        .iter()
        .filter(|item| matches!(item.kind, ItemKind::Comment(_)))
        .collect();
    
    assert_eq!(comment_items.len(), 0, "Comments should not appear as items");
    
    // Should still have the non-comment items
    let const_items: Vec<_> = program.items
        .iter()
...
# Comment 3"#;
    ...
    let comment_contents: Vec<&str> = program.debug_info.comments
        .iter()
        .map(|c| c.content.as_str())
        .collect();
    
    assert_eq!(comment_contents, vec![" Comment 1", " Comment 2  ", " Comment 3"]);
}
