...
impl OutrunParser {
    ...
    fn extract_inner_with_text(pair: pest::iterators::Pair<Rule>) -> ParseResult<(pest::iterators::Pair<Rule>, String, Span)> {
    ...
    fn unexpected_token_from_pair(pair: &pest::iterators::Pair<Rule>, expected: &str) -> ParseError {
                ...
                Rule::program => {
                    ...
                    for inner_pair in pair.into_inner() {
                        ...
                        match inner_pair.as_rule() {
                            Rule::keyword => {
                                parse_and_wrap_item!(items, inner_pair, pair_span, parse_keyword, Keyword);
                            }
                            Rule::function_definition => {
                                parse_and_wrap_item!(items, inner_pair, pair_span, parse_function_definition, FunctionDefinition);
                            }
                            Rule::struct_definition => {
                                parse_and_wrap_item!(items, inner_pair, pair_span, parse_struct_definition, StructDefinition);
                            }
                            Rule::trait_definition => {
                                parse_and_wrap_item!(items, inner_pair, pair_span, parse_trait_definition, TraitDefinition);
                            }
                            Rule::impl_block => {
                                parse_and_wrap_item!(items, inner_pair, pair_span, parse_impl_block, ImplBlock);
                            }
                            Rule::alias_definition => {
                                parse_and_wrap_item!(items, inner_pair, pair_span, parse_alias_definition, AliasDefinition);
                            }
                            Rule::import_definition => {
                                parse_and_wrap_item!(items, inner_pair, pair_span, parse_import_definition, ImportDefinition);
                            }
                            ...
                            Rule::boolean => {
                                parse_and_wrap_item!(items, inner_pair, pair_span, parse_boolean, BooleanLiteral);
                            }
                            Rule::float => {
                                parse_and_wrap_item!(items, inner_pair, pair_span, parse_float, FloatLiteral);
                            }
                            Rule::string => {
                                parse_and_wrap_item!(items, inner_pair, pair_span, parse_string, StringLiteral);
                            }
                            Rule::atom => {
                                parse_and_wrap_item!(items, inner_pair, pair_span, parse_atom, AtomLiteral);
                            }
                            Rule::sigil => {
                                parse_and_wrap_item!(items, inner_pair, pair_span, parse_sigil, SigilLiteral);
                            }
                            Rule::list => {
                                parse_and_wrap_item!(items, inner_pair, pair_span, parse_list, ListLiteral);
                            }
                            Rule::map => {
                                parse_and_wrap_item!(items, inner_pair, pair_span, parse_map, MapLiteral);
                            }
                            Rule::tuple => {
                                parse_and_wrap_item!(items, inner_pair, pair_span, parse_tuple, TupleLiteral);
                            }
                            Rule::integer => {
                                parse_and_wrap_item!(items, inner_pair, pair_span, parse_integer, IntegerLiteral);
                            }
                            Rule::identifier => {
                                parse_and_wrap_item!(items, inner_pair, pair_span, parse_identifier, Identifier);
                            }
                            Rule::type_identifier => {
                                parse_and_wrap_item!(items, inner_pair, pair_span, parse_type_identifier, TypeIdentifier);
                            }
                            Rule::comment => {
                                parse_and_wrap_item!(items, inner_pair, pair_span, parse_comment, Comment);
                            }
                            Rule::block_comment => {
                                parse_and_wrap_item!(items, inner_pair, pair_span, parse_block_comment, Comment);
                            }
                            Rule::newline => {
    ...
    fn parse_qualified_identifier(pair: pest::iterators::Pair<Rule>) -> ParseResult<QualifiedIdentifier> {
        let span = Self::span_from_pair(&pair);
    ...
    fn parse_struct_pattern(pair: pest::iterators::Pair<Rule>) -> ParseResult<StructPattern> {
        ...
        Ok(StructPattern { type_name, fields, span })
    }
    ...
    fn parse_list_pattern(pair: pest::iterators::Pair<Rule>) -> ParseResult<ListPattern> {
        ...
        Ok(ListPattern { elements, rest, span })
    }
    ...
    fn parse_macro_injection(
        pair: pest::iterators::Pair<Rule>,
    ) -> ParseResult<MacroInjection> {
        let span = Self::span_from_pair(&pair);
        let mut inner_pairs = pair.into_inner();
